package com.psiphiglobal.identity.logging;
...
public class ResponseLogger implements ContainerResponseFilter
{
    ...
    private static final String RESPONSE_LOG_FORMAT = "[RESPONSE] %s %dms %d >>>> %s";
    private static final String RESPONSE_LOG_FORMAT_NO_BODY = "[RESPONSE] %s %dms %d";
    ...
    private String formatLog(String requestId, int httpStatus, long timeTaken, Object body)
    {
        if (body == null)
        {
            return String.format(RESPONSE_LOG_FORMAT_NO_BODY, requestId, timeTaken, httpStatus);
        }
        else
        {
            return String.format(RESPONSE_LOG_FORMAT, requestId, timeTaken, httpStatus, body);
        }
    }
}
